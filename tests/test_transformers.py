...
def test_delay_no_lift_input():
    ...
    # Instantiate lifting function
    lf = lifting_functions.DelayLiftingFn(lift_input=False,
                                          n_delays_state=1,
                                          n_delays_input=1)
    lf.fit(X, n_inputs=2, episode_feature=False)
    # Check attributes
    assert lf.n_delays_input == 1
    assert lf.n_delays_input_ == 0
    assert lf.n_inputs_in_ == 2
    assert lf.n_inputs_out_ == 2
    # Check forward transform
    Xd_fit = lf.transform(X)
    np.testing.assert_allclose(Xd_exp, Xd_fit)
    # Check inverse transform
